...
func (a *API) CheckConfiguration(config interface{}) error {
	...
	if aConfig.Drivers.Local.Enabled && aConfig.Auth.Local.SigninEnabled && (aConfig.Auth.Local.Organization == "" || !aConfig.Auth.AllowedOrganizations.Contains(aConfig.Auth.Local.Organization)) {
		return errors.New("local authentication driver organization empty or not allowed in field 'allowedOrganizations'")
	}
	if aConfig.Drivers.OIDC.Enabled && aConfig.Auth.OIDC.SigninEnabled && (aConfig.Auth.OIDC.Organization == "" || !aConfig.Auth.AllowedOrganizations.Contains(aConfig.Auth.OIDC.Organization)) {
		return errors.New("oidc authentication driver organization empty or not allowed in field 'allowedOrganizations'")
	}
	if aConfig.Drivers.Gitlab.Enabled && aConfig.Auth.Gitlab.SigninEnabled && (aConfig.Auth.Gitlab.Organization == "" || !aConfig.Auth.AllowedOrganizations.Contains(aConfig.Auth.Gitlab.Organization)) {
		return errors.New("gitlab authentication driver organization empty or not allowed in field 'allowedOrganizations'")
	}
	if aConfig.Drivers.Github.Enabled && aConfig.Auth.Github.SigninEnabled && (aConfig.Auth.Github.Organization == "" || !aConfig.Auth.AllowedOrganizations.Contains(aConfig.Auth.Github.Organization)) {
...
func (a *API) Serve(ctx context.Context) error {
	...
	a.ExternalDrivers = make([]string, 0)

	a.AuthenticationDrivers[sdk.ConsumerBuiltin] = builtin.NewDriver()
	if a.Config.Drivers.Local.Enabled && a.Config.Auth.Local.SigninEnabled {
	...
	if a.Config.Drivers.LDAP.Enabled {
		a.ExternalDrivers = append(a.ExternalDrivers, string(sdk.ConsumerLDAP))
	...
	if a.Config.Drivers.Github.Enabled {
		a.ExternalDrivers = append(a.ExternalDrivers, string(sdk.ConsumerGithub))
	...
	if a.Config.Drivers.Gitlab.Enabled {
		a.ExternalDrivers = append(a.ExternalDrivers, string(sdk.ConsumerGitlab))
	...
	if a.Config.Drivers.OIDC.Enabled {
		a.ExternalDrivers = append(a.ExternalDrivers, string(sdk.ConsumerOIDC))
	...
	if a.Config.Drivers.CorporateSSO.Enabled && a.Config.Auth.CorporateSSO.SigninEnabled {
